	...
	if (hierarchy.size() > 0)
	{
		...
		if (numObjects < 3)
		{
			for (int i = 0; i >= 0; i = hierarchy[i][0])
			{
				...
				if (area > MIN_AREA)
				{
					tempX = moment.m10 / area;//Player X
					tempY = moment.m01 / area;//Player Y
				}
			}
			if (ControllerType == "playerOne")
			{
				location[0] = tempX;//Left player X
				location[1] = tempY;//Left player Y
			}
			if (ControllerType == "playerTwo")
			{
				location[4] = tempX;//Right player X
				location[5] = tempY;//Right player Y
			}
		}
	}
...
Export  char* OpenCVData(char * PlayerData)
{
